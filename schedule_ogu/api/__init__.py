...
from schedule_ogu.utils import ScheduleTime
from schedule_ogu.models import UserAgentModel, CookieModel

from .models import (ScheduleEntryHTTP,
                     StudentGroupHTTP,
                     FacultyHTTP,
                     DepartmentHTTP,
                     EmployeeHTTP, ScheduleHTTP, ExamHTTP)
...
class HTTPClient:
    ...
    async def update_cookies(self):
        ...
        try:
            ...
            logger.info("Get cookies %s", cookies)
    ...
    async def get_schedule_student(self, group_id: int, week: int = None) -> t.Optional[ScheduleHTTP]:
        ...
        if not data:
            return
        return ScheduleHTTP(sorted([ScheduleEntryHTTP.parse_obj(raw) for index, raw in data.items()
                                    if index.isdigit()],
                                   key=lambda x: x.date))

    async def get_schedule_employee(self, employee_id: int, week: int = None) -> t.Optional[ScheduleHTTP]:
        timestamp = ScheduleTime.compute_timestamp_for_site(week)
    ...
    async def get_departments(self, faculty_id: int) -> t.List[DepartmentHTTP]:
        route = Route('GET', '/schedule/{faculty_id}/kaflist',
                      faculty_id=faculty_id)
        data = await self.request(route)
